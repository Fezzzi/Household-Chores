...
exports.up = (pgm: MigrationBuilder) => {
  ...
  pgm.sql(`
    CREATE TABLE ${tHouseMemName} (
      ${tHouseMemCols.household_id} INTEGER NOT NULL REFERENCES ${tHouseholdsName} (${tHouseholdsCols.id}) ON DELETE CASCADE,
      ${tHouseMemCols.user_id} INTEGER NOT NULL REFERENCES ${tUsersName} (${tUsersCols.id}) ON DELETE CASCADE,
      ${tHouseMemCols.from_id} INTEGER NOT NULL REFERENCES ${tUsersName} (${tUsersCols.id}) ON DELETE CASCADE,
  ...
  pgm.sql(`
    CREATE TABLE ${tHouseInvName} (
      ${tHouseInvCols.household_id} INT NOT NULL REFERENCES ${tHouseholdsName} (${tHouseholdsCols.id}) ON DELETE CASCADE,
      ${tHouseInvCols.from_id} INT NOT NULL REFERENCES ${tUsersName} (${tUsersCols.id}) ON DELETE CASCADE,
      ${tHouseInvCols.to_id} INT NOT NULL REFERENCES ${tUsersName} (${tUsersCols.id}) ON DELETE CASCADE,
