...
export const isDialogColumnKey = (key: string): key is DialogSettingsColumn =>
  key in Object.values(tDialogsCols).filter(col => col !== tDialogsCols.user_id)

export const disableDialog = async (dialogColumn: DialogSettingsColumn, userId: number) =>
  database.queryBool(`
    UPDATE ${tDialogsName} SET ${dialogColumn}=1
    WHERE ${tDialogsCols.user_id}=${userId}
  `)

export const updateDialogSettings = async (data: Record<DialogSettingsColumn, number>, userId: number) =>
  database.queryBool(`
    UPDATE ${tDialogsName}
